    ...
    CSV.prototype.parse = function(callback) {
      ...
      if (!callback) {
        ...
        callback = function(record) {
          ...
          for(var k in record){
            record[k] = types[k](record[k]);
          }
          response[record[key]] = record;
        }
      }
      ...
      function saveLine() {
        if (header) {
          if (isArray(header)) {
            ...
            header = [];
            types = {};
            forEach(current.line, function(value, index){
              var idx = value.lastIndexOf("_");
              var val = value;
              if(idx > 0) {
                val = value.substring(0, idx);
              }
              var type = Number;
              if(idx >= 0) {
                var flag = value.substring(idx+1)
                if(flag.match("k")) {
                  key = val;
                }
                ...
                if(flag.match("n")) {
                  ...
                  type = function(tvalue) { return tvalue; };
                }
                else if(flag.match("c")) {
                  type = function(tvalue) { return null; };
                }
              }
              header.push(val);
              types[val] = type;
            });
            if(key == null && isArray(header)) {
              key = header[0];
            }
          }
        } else {
